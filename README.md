...
이 한 줄로 자동 수행:
`.env`/TLS/OpenBao 초기화 → AppRole(read-only 정책) + Audit log → 서비스 4개(OpenBao·PG·LiteLLM·Nginx) 기동 → 사용자 등록 → 24h Virtual Key 일괄 발급 → 8개 통합 테스트 → 요약 출력.
...
stdin 입력(셸 히스토리 회피): `./scripts/set-openai-key.sh user01 -` 후 키 붙여넣기 + Ctrl-D.

상세 — IP 화이트리스트 / Key 회전 / `bao kv put` 직접 사용: [admin-guide.md §4](docs/guides/admin-guide.md).

### 사용자 메타데이터 + IP 바인딩

[config/users.conf](config/users.conf.example) — `SLOT|EMAIL|NAME[|ALLOWED_IPS]`:

```bash
USERS=(
  "user01|alice@company.com|홍길동"
  "user02|bob@company.com|김철수|10.0.1.0/24"        # IP 바인딩 (옵션)
  "user03|carol@company.com|이영희|10.0.2.5,10.0.2.6"
  ...
)
```

편집 후 `./start.sh` 재실행하면 LiteLLM 사용자 + Virtual Key가 갱신된다(allowed_ips 적용).
...
├── .env / .env.example              # 자동 생성, chmod 600 (OpenAI Key는 여기 없음)
├── config/users.conf                # 사용자 메타데이터 (slot/email/name/allowed_ips)
├── secrets/                         # 런타임 시크릿 (gitignore, chmod 700)
│   ├── openbao-approle.env          #   AppRole role-id + secret-id
...
├── scripts/
│   ├── set-openai-key.sh            # ★ OpenAI Key를 OpenBao에 직접 적재 + 자동 reload
│   ├── 01-init-openbao.sh           # 초기화 + AppRole 정책 + Audit (멱등)
│   ├── 02-load-secrets.sh           # OpenBao→secrets/litellm-secrets.env 미러 (AppRole)
│   ├── 03-register-users.sh         # 사용자 등록 + Virtual Key 발급 (allowed_ips 포함)
│   ├── 04-health-check.sh           # LiteLLM readiness 대기
│   └── sample-keys.txt              # 발급된 Virtual Key (자동 생성, gitignore)
├── tests/                           # 8개 통합 테스트 + test-all.sh
└── docs/
    ├── guides/
    ...
    └── design/
        ...
        └── D4-reference-gap-analysis.md # 참조 아키텍처 갭 분석 + WI-1~6 개선 설계
```
...
```
[브라우저/Codex]──HTTPS──►[Nginx :443 + X-Forwarded-For]──►[LiteLLM :4000]
                                                                  │
                                                                  ▼
                                            ┌───────────┴───────────┐
                                            ▼                       ▼
                                     [PostgreSQL :5432]    [OpenBao :8200]
                                     Users/Keys/Spend      KV (root only WRITE,
                                     90d retention         AppRole READ)
                                                           audit.log
```

- 인증: Virtual Key(`sk-vk-…`, 24h TTL, `allowed_ips` 옵션) → LiteLLM이 슬롯의 OpenAI Key로 치환 후 OpenAI 호출
- 시크릿 분리: OpenAI Key는 OpenBao SSOT. LiteLLM은 AppRole(read-only 정책)로 접근. root token은 부트스트랩에만 사용.
- 사용자 간 격리: `userNN-gpt-4o` 모델은 `userNN`만 호출 가능 (RBAC + 모델 화이트리스트)
- 감사: 모든 KV read/write가 OpenBao audit log에 HMAC된 토큰 accessor로 기록
- 관리자 UI: Master Key 단독 로그인, 전체 prompt/spend Logs 열람
...
| Audit 로그 보기 | `tail -F openbao/logs/audit.log` |
| 새 Virtual Key 일괄 재발급 | `./start.sh` (멱등 — 매일 1회 권장) |
| 통합 테스트만 재실행 | `./tests/test-all.sh` |
| OpenBao Web UI 접속 | SSH `-L 8200:127.0.0.1:8200` 후 `http://localhost:8200/ui` (admin-guide §10) |
...
| Phase 4 `AppRole login failed` | `rm secrets/openbao-approle.env && ./start.sh` |
| 사용자 401 | Virtual Key 24h 만료 또는 allowed_ips 불일치 — 새 키 배포 |
| OpenAI 401 | OpenBao 슬롯이 placeholder — `./scripts/set-openai-key.sh userNN sk-proj-...` |
| OpenBao Sealed | `./openbao/unseal.sh` |

전체 표 → [admin-guide.md §14](docs/guides/admin-guide.md).
