...
## 3. 새 구조 — Root CA + 서명된 leaf + 명확한 디렉토리 분리

```
                        ┌─────────────────────────────┐
                        │ dev/local-root-ca.crt       │   ← OS trust store 에 등록
   trust 등록 대상 →   │ (CN=Local Dev Root CA,      │     (sudo update-ca-certificates)
                        ...
                        └──────────────┬──────────────┘
                                        ...
                                        ▼
                        ┌─────────────────────────────┐
                        │ dev/server.crt              │   ← Nginx가 들이미는 leaf
   Nginx end-entity →   │ (CN=localhost,              │      (server.crt symlink → 여기)
                        │  CA:FALSE,                  │
                        │  EKU: serverAuth,           │
                        │  SAN: DNS:localhost,        │
                        │       IP:127.0.0.1)         │
...
└── dev/                ← 로컬 테스트 산출물 (gitignore, 운영과 격리)
    ├── local-root-ca.crt        ← OS trust store 등록 대상
    ├── local-root-ca.key        chmod 600
    ├── local-root-ca.srl        openssl 시리얼 카운터
    ├── server.crt               dev leaf cert (실체)
    ├── server.key               chmod 600
    ├── server.csr               CSR
    ├── server-openssl.cnf       CSR config
    └── server-ext.cnf           서명 extensions (SAN/EKU/Basic)
```

**구분 원칙**: 운영 cert이 들어갈 자리는 항상 `nginx/certs/server.crt` 와 `server.key`. 사내 CA 발급 cert으로 교체할 때 그 자리에 그대로 덮어쓰면 됨. dev 모드일 때는 그 자리에 `dev/`로의 symlink가 있고, 운영 cert으로 교체되면 일반 파일이 된다.

| 파일 | 역할 | trust store 등록? |
|---|---|---|
| `nginx/certs/server.crt`, `server.key` | Nginx가 실제 로딩 — dev에서는 symlink, 운영에서는 사내 CA cert | **✗** |
| `nginx/certs/dev/local-root-ca.crt` | Local Dev Root CA (CA:TRUE) | **✓ 등록** |
| `nginx/certs/dev/local-root-ca.key` | Root CA 개인키 (chmod 600) | ✗ |
| `nginx/certs/dev/server.crt` | dev leaf cert (CA:FALSE, CA로 서명됨) | **✗ 절대 등록 금지** |
| `nginx/certs/dev/server.key` | dev 서버 개인키 (chmod 600) | ✗ |
| `nginx/certs/dev/server.csr` | CSR | ✗ |
| `nginx/certs/dev/server-openssl.cnf` | CSR config | ✗ |
| `nginx/certs/dev/server-ext.cnf` | 서명 extensions (SAN/EKU/Basic) | ✗ |
| `nginx/certs/dev/local-root-ca.srl` | openssl 시리얼 카운터 | ✗ |

> ⚠️ **`dev/server.crt` 를 trust store 에 등록하지 말 것.**
> 등록하면 OS는 그것을 "신뢰하는 CA" 로 취급하는데 동시에 같은 cert 이 Nginx 서버 cert 으로 들이밀리므로 `CaUsedAsEndEntity` 가 되돌아온다. 등록 대상은 **항상 `dev/local-root-ca.crt` 한 장만**.
...
cert 자체 검사 (symlink를 통해 dev/server.crt 가 평가됨 — 호스트에서 `nginx/certs/server.crt`로 그대로 접근 가능):

```bash
# 0) 레이아웃 한눈에 — dev 모드 여부 즉시 판별
ls -la nginx/certs/
# 기대:
#   server.crt -> dev/server.crt
#   server.key -> dev/server.key
#   dev/                   (디렉토리)
...
openssl verify -CAfile nginx/certs/dev/local-root-ca.crt nginx/certs/server.crt
...
```bash
sudo cp nginx/certs/dev/local-root-ca.crt /usr/local/share/ca-certificates/llm-access-gateway-local-root-ca.crt
sudo update-ca-certificates
```

다른 OS:
- **macOS**: `sudo security add-trusted-cert -d -r trustRoot -k /Library/Keychains/System.keychain nginx/certs/dev/local-root-ca.crt`
- **Fedora/RHEL**: `sudo cp nginx/certs/dev/local-root-ca.crt /etc/pki/ca-trust/source/anchors/ && sudo update-ca-trust`
- **Arch**: `sudo trust anchor nginx/certs/dev/local-root-ca.crt`

WSL Ubuntu 에서 Windows 의 사내 인증서를 쓰고 싶다면 Windows trust 와는 별개로 WSL 안에서 위 명령을 따로 실행해야 한다.

### Q. Codex 가 `CaUsedAsEndEntity` 를 낸다

→ Nginx가 들이미는 cert이 CA 플래그를 들고 있다는 뜻. 다음을 확인:
...
`CA:TRUE` 가 보이면 옛 self-signed cert 가 남아있는 것. dev 모드라면 강제 재생성:

```bash
rm -rf nginx/certs/dev nginx/certs/server.crt nginx/certs/server.key
./start.sh
```
...
1. dev 모드 symlink 제거:
   ...
   ```
2. 사내 CA 가 서명한 fullchain → `nginx/certs/server.crt` (실제 파일로 복사)
3. 그에 대응하는 개인키 → `nginx/certs/server.key` (chmod 600)
4. `./start.sh` 재실행 — `is_production_cert` 가 "symlink 아님 + issuer != Local Dev Root CA"를 보고 자동 보존 (덮어쓰지 않음).

start.sh 의 판정 기준:
- `nginx/certs/server.crt` 가 **symlink** → dev 모드, dev cert chain 검증/재생성
- `nginx/certs/server.crt` 가 **일반 파일** + issuer = `Local Dev Root CA` → 우리 cert을 사용자가 복사해 둔 상태, dev로 분류
- `nginx/certs/server.crt` 가 **일반 파일** + issuer ≠ `Local Dev Root CA` → 운영 cert, 자동 보존

`nginx/certs/dev/` 디렉토리는 운영 모드에서도 남아 있지만 nginx가 참조하지 않으므로 무시된다. 깔끔하게 지우고 싶으면 `rm -rf nginx/certs/dev/`. 단 dev 모드로 다시 돌릴 때는 `./start.sh`가 다시 생성한다.

---

## 9. 보안 주의

- `dev/local-root-ca.key`, `dev/server.key`, 그리고 `nginx/certs/server.key` (운영 cert일 때) 는 `chmod 600`. 절대 commit 금지 (`.gitignore`가 `nginx/certs/dev/` 전체와 `server.key`/`server.crt`를 제외).
- `dev/local-root-ca.crt` 는 *개발자 본인 머신에서만* 신뢰돼야 한다. 다른 사람의 trust store 에 배포하지 말 것 — 그 사람의 모든 HTTPS 트래픽에 대해 당신이 leaf cert 을 발급할 수 있는 권한이 생긴다.
