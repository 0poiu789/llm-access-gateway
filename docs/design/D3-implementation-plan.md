...
⚠️ **중요 제약**: LiteLLM OSS는 Hashicorp Vault 연동(`key_management_system: "hashicorp_vault"`)을 지원하지 않는다 — 이는 LiteLLM Enterprise 유료 기능이다. 따라서 본 PoC는 다음 구조를 채택한다.

```
OpenBao (master)  ──(start.sh가 읽음)──►  .env  ──(docker-compose 주입)──►  LiteLLM 컨테이너 OS env
   │                                                                              │
   │                                                                              ▼
   │                                                            api_key: "os.environ/USERnn_OPENAI_KEY"
   └─────── 시크릿 저장/회전/감사의 source of truth ───────────────────────► (OS env 직접 조회)
```

- **OpenBao**: 시크릿의 source of truth. 키 회전, 감사 로그, 접근 제어 담당.
- **`.env`**: OpenBao의 캐시. `start.sh`가 매 실행 시 OpenBao에서 읽어 갱신한다.
- **LiteLLM**: 컨테이너 시작 시 OS env에서 키를 읽음. vault SDK 호출 없음.

**키 회전 절차:**
1. OpenBao에서 `secret/litellm/USER01_OPENAI_KEY` 갱신
2. `./start.sh` 재실행 (또는 `bash scripts/02-load-secrets.sh && docker compose up -d --force-recreate litellm`)

LiteLLM Enterprise를 도입하면 `litellm/config.yaml`의 `key_management_system` 항목을 복원하여 vault 직접 연동으로 전환할 수 있다.
